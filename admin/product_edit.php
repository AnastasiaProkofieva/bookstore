...
if (!empty($_POST)){
    $productService->update($_GET['id'],$_POST);
}
$book =$productService->getBookById($_GET['id']);
$genreSersive = new GenreService();
$genrelist= $genreSersive->getGenreStats();
?>
                        ...
                        <div class="col-lg-12">
                            <div class="p-5">
                                ...
                                <form class="user" method="post">
                                    ...
                                    <div class="form-group">
                                        <input type="text" name="author" value="<?=$book['authorName']?>" class="form-control form-control-user" id="exampleInputEmail" aria-describedby="emailHelp" placeholder="Автор">
                                    </div>
                                    <div class="form-group">
                                        <select id="inputState" name= "genre" class="form-control">
                                            <option selected>Выберите жанр...</option>
                                            <?php foreach ($genrelist as $genre):?>
                                            <option <?=$genre['genre_name']==$book['genre_id'] ? 'selected': ''?>><?=$genre['genre_name']?></option>
                                            <?php endforeach;?>
                                        </select>
                                    </div>
